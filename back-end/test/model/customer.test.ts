...
const customer = new Customer(id, firstName, lastName, email, birthday, password);
...
test('given: invalid firstName, when: setFirstName, then: throw error', () => {
    const customer = () => 
        new Customer(id, "", lastName, email, birthday, password)
...
test('given: invalid lastName, when: setLastName, then: throw error', () => {
    const customer = () => 
        new Customer(id, firstName, "", email, birthday, password)
...
test('given: invalid email, when: setEmail, then: throw error', () => {
    const customer = () =>
        new Customer(id, firstName, lastName, "invalid-email", birthday, password)
  expect(customer).toThrow("Invalid email format.");
});

test('given: invalid password, when: setPassword, then: throw error', () => {
    const customer = () =>
        new Customer(id, firstName, lastName, email, birthday, "short")
...
test('given: invalid birthday, when: setBirthday, then: throw error', () => {
    const customer = () =>
        new Customer(id, firstName, lastName, email, undefined as unknown as Date, password)
